...
def train(model, train_loader, val_loader, learning_rate=0.01):
    ...
    for epoch in range(1, 1 + epochs):
        ...
        model.train()
        model.set_teacher_forcing(ratio=0.5)
        for batch in train_epoch:
            ...
            loss, ppl = model(src_tensor, src_len, tgt_tensor, tgt_len)
            loss.backward()
            optimizer.step()
            ...
            train_epoch.set_postfix(loss=loss.item(), ppl=ppl.item())
        ...
        with torch.no_grad():
            for batch in val_epoch:
                ...
                val_epoch.set_postfix(loss=loss.item(), ppl=ppl.item())

        val_loss /= len(val_loader)
        val_ppl /= len(val_loader)
        print(f"Epoch [{epoch}/{epochs}]: Train loss: {train_loss}, Train ppl: {train_ppl},  Valid loss: {val_loss}, Valid ppl: {val_ppl}")
...
model = NMT(source_vocab, target_vocab, embed_size, hidden_size, device, layer, dropout=dropout).to(device)

"""
    1. layer=2  embed_size=300  hidden_size=300 lr=0.001 MAX_LENGTH=50
        BLEU SCORE: BLEU = 24.95 59.5/32.8/20.1/12.8 (BP = 0.938 ratio = 0.940 hyp_len = 132514 ref_len = 141043)
"""
if args.mode == 'train':
    train(model, train_dataloader, val_dataloader, learning_rate=lr)
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():
        for batch in test_epoch:
            src_tensor, src_len, tgt_tensor, tgt_len = batch
            tokens = model.beam_search_generate(src_tensor, src_len, beam_size=5, max_len=MAX_LENGTH)
            source_sentence = " ".join(source_vocab.convert_tokens_to_words(src_tensor[0].tolist()))
