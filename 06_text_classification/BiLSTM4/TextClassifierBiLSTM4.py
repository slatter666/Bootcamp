...
class TextClassifierBiLSTM4(TextClassifier):
    def __init__(self, train_path, model_path, embed_dim, hidden_dim, num_layers, num_class):
        super(TextClassifierBiLSTM4, self).__init__()
        self.processer = DataProcess(train_path, val_path='', test_path='')  # 主要是为了获得其中的词汇表进行tensor转换
        self.model = BiLSTMModel4(vocab_size=len(self.processer.vocab), embed_size=embed_dim, hidden_size=hidden_dim,
...
if __name__ == '__main__':
    ...
    model_param_path = 'checkpoint_model/batch=64-embed=300-hidden=20-layer=3/epoch=0-train_loss=0.44-train_F1=0.71-val_loss=0.38-val_acc=0.86-val_precision=0.81-val_recall=0.92-val_F1=0.86.ckpt'
    classifier = TextClassifierBiLSTM4(train_data_path, model_param_path, _embed_dim, _hidden_dim, _num_layers,_num_class)
