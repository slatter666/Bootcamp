"""
    使用BiLSTM进行分类, 用双向LSTM最后得到的hidden作为输入进行分类预测
    效果最好的模型参数为embed=500, hidden=120, layer=3
    大概达到 val_acc=0.87, val_F1=0.86, test_F1=0.80
    不过实际上训练效果并不那么好的模型embed=120,hidden=20,layer=3 选取模型参数为val_F1=0.82的模型  最终test_F1可以达到0.82

    尝试把单层fc改为MLP之后其实效果也差不多, 该模型的极限大致就是这么多了, 设计另一个模型使用所有的hidden输出来进行预测而非最终的hidden
    个人认为test_set和train_set、val_set的数据分别差异还是稍微有点大
"""
import torch
...
class BiLSTMModel1(pl.LightningModule):
    ...
    def forward(self, padded_text, actual_length):
        ...
        ###       - Note that the shape of the ht tensor returned by the bilstm is (2 * num_layers, b, h)
        ###     3.`ht` is a tensor shape (2 * num_layers, b, h). We need to concatenate the last layer's forwards and
        ###        backwards tensors to obtain a tensor shape (b, 2 * h). Then apply the fc layer to this inorder to
        ###        predict the label
        ...
        result = self.fc(torch.cat((ht[-2], ht[-1]), dim=1))
        return result
