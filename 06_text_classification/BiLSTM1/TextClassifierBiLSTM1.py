...
class TextClassifierBiLSTM1(TextClassifier):
    def __init__(self, train_path, model_path, embed_dim, hidden_dim, num_layers, num_class):
        super(TextClassifierBiLSTM1, self).__init__()
        self.processer = DataProcess(train_path, val_path='', test_path='')  # 主要是为了获得其中的词汇表进行tensor转换
        self.model = BiLSTMModel1(vocab_size=len(self.processer.vocab), embed_size=embed_dim, hidden_size=hidden_dim,
...
if __name__ == '__main__':
    ...
    model_param_path = 'checkpoint_model/batch=128-embed=500-hidden=120-layer=3/epoch=2-train_loss=0.12-train_F1=1.00-val_loss=0.33-val_acc=0.87-val_precision=0.88-val_recall=0.85-val_F1=0.86.ckpt'
    classifier = TextClassifierBiLSTM1(train_data_path, model_param_path, _embed_dim, _hidden_dim, _num_layers,_num_class)
