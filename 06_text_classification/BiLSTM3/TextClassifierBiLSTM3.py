...
from BiLSTMModel3 import BiLSTMModel3
...
class TextClassifierBiLSTM3(TextClassifier):
    def __init__(self, train_path, model_path, embed_dim, hidden_dim, num_layers, num_class):
        super(TextClassifierBiLSTM3, self).__init__()
        self.processer = DataProcess(train_path, val_path='', test_path='')  # 主要是为了获得其中的词汇表进行tensor转换
        self.model = BiLSTMModel3(vocab_size=len(self.processer.vocab), embed_size=embed_dim, hidden_size=hidden_dim,
...
if __name__ == '__main__':
    ...
    model_param_path = 'checkpoint_model/batch=64-embed=250-hidden=30-layer=3/epoch=1-train_loss=0.33-train_F1=0.90-val_loss=0.38-val_acc=0.83-val_precision=0.76-val_recall=0.92-val_F1=0.83.ckpt'
    classifier = TextClassifierBiLSTM3(train_data_path, model_param_path, _embed_dim, _hidden_dim, _num_layers,_num_class)
